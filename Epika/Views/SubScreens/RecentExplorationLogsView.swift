...
private struct EncounterDetailView: View {
        ...
        Group {
            if encounter.combatSummary == nil {
                ...
                ForEach(actionItems, id: \.id) { item in
                    BattleActionSection(item: item, iconInfo: actorIcons[item.actorId])
                }
            }
        }
    }
    ...
    private func buildActionItems() -> [ActionItem] {
        ...
        // 初期HPを構築
        var hpState: [UInt16: Int] = [:]
        var maxHPState: [UInt16: Int] = [:]
        var nameState: [UInt16: String] = [:]
        ...
        for (index, participant) in archive.playerSnapshots.enumerated() {
            ...
            hpState[actorIndex] = Int(initialHP)
            maxHPState[actorIndex] = participant.maxHP
            nameState[actorIndex] = participant.name
        }
        ...
        for action in archive.battleLog.actions {
            ...
            // 表示対象外のアクションはスキップ
            if !shouldDisplayActionKind(kind) { continue }

            let actorName = nameState[action.actor] ?? "不明"
            let actorHP = hpState[action.actor] ?? 0
            let actorMaxHP = maxHPState[action.actor] ?? 1

            let message = makeMessage(kind: kind, actorName: actorName, action: action, nameState: nameState)
            ...
            // HPを更新
            applyHPChange(action: action, kind: kind, hpState: &hpState, maxHPState: maxHPState)
        }

        return items
    }

    private func shouldDisplayActionKind(_ kind: ActionKind) -> Bool {
        switch kind {
        case .battleStart, .turnStart, .enemyAppear:
            return false
        default:
            return true
        }
    ...
    private func makeMessage(kind: ActionKind, actorName: String, action: BattleAction, nameState: [UInt16: String]) -> String {
        let targetName = action.target.flatMap { nameState[$0] } ?? "対象"
        let value = action.value.map { Int($0) } ?? 0

        switch kind {
        case .defend:
            return "\(actorName)は防御態勢を取った"
        case .physicalAttack:
            return "\(actorName)の攻撃！"
        case .priestMagic:
            return "\(actorName)は回復魔法を唱えた！"
        case .mageMagic:
            return "\(actorName)は攻撃魔法を唱えた！"
        case .breath:
            return "\(actorName)はブレスを吐いた！"
        case .victory:
            return "勝利！ 敵を倒した！"
        case .defeat:
            return "敗北… パーティは全滅した…"
        case .retreat:
            return "戦闘は長期化し、パーティは撤退を決断した"
        case .physicalDamage, .magicDamage, .breathDamage:
            return "\(targetName)に\(value)のダメージ！"
        case .physicalEvade:
            return "\(targetName)は攻撃をかわした！"
        case .physicalParry:
            return "\(actorName)の受け流し！"
        case .physicalBlock:
            return "\(actorName)は大盾で防いだ！"
        case .physicalKill:
            return "\(targetName)を倒した！"
        case .martialArts:
            return "\(actorName)の格闘戦！"
        case .magicHeal, .healParty:
            return "\(targetName)のHPが\(value)回復！"
        case .magicMiss:
            return "しかし効かなかった"
        case .statusInflict:
            return "\(targetName)は状態異常になった！"
        case .statusResist:
            return "\(targetName)は抵抗した！"
        case .statusRecover:
            return "\(targetName)の状態異常が治った"
        case .statusTick:
            return "\(targetName)は継続ダメージで\(value)のダメージ！"
        case .statusConfusion:
            return "\(actorName)は暴走して混乱した！"
        case .statusRampage:
            return "\(actorName)の暴走！"
        case .reactionAttack:
            return "\(actorName)の反撃！"
        case .followUp:
            return "\(actorName)の追加攻撃！"
        case .healAbsorb:
            return "\(actorName)は吸収能力で\(value)回復"
        case .healVampire:
            return "\(actorName)は吸血で\(value)回復"
        case .healSelf:
            return "\(actorName)は自身の効果で\(value)回復"
        case .damageSelf:
            return "\(actorName)は自身の効果で\(value)ダメージ"
        case .buffApply:
            return "効果が発動した"
        case .buffExpire:
            return "\(actorName)の効果が切れた"
        case .resurrection:
            return "\(targetName)が蘇生した！"
        case .necromancer:
            return "\(actorName)のネクロマンサーで\(targetName)が蘇生した！"
        case .rescue:
            return "\(actorName)は\(targetName)を救出した！"
        case .actionLocked:
            return "\(actorName)は動けない"
        case .noAction:
            return "\(actorName)は何もしなかった"
        case .withdraw:
            return "\(actorName)は戦線離脱した"
        case .sacrifice:
            return "古の儀：\(targetName)が供儀対象になった"
        case .vampireUrge:
            return "\(actorName)は吸血衝動に駆られた"
        default:
            return ""
        }
    }

    private func applyHPChange(action: BattleAction, kind: ActionKind, hpState: inout [UInt16: Int], maxHPState: [UInt16: Int]) {
        let value = Int(action.value ?? 0)

        switch kind {
        // ダメージ（ターゲットのHPを減らす）
        case .physicalDamage, .magicDamage, .breathDamage, .statusTick:
            if let target = action.target, let current = hpState[target] {
                hpState[target] = max(0, current - value)
            }
        // 自傷ダメージ
        case .damageSelf:
            if let current = hpState[action.actor] {
                hpState[action.actor] = max(0, current - value)
            }
        // 回復（ターゲットのHPを増やす）
        case .magicHeal, .healParty:
            if let target = action.target,
               let current = hpState[target],
               let maxHP = maxHPState[target] {
                hpState[target] = min(maxHP, current + value)
            }
        // 自己回復
        case .healAbsorb, .healVampire, .healSelf:
            if let current = hpState[action.actor],
               let maxHP = maxHPState[action.actor] {
                hpState[action.actor] = min(maxHP, current + value)
            }
        // 戦闘不能
        case .physicalKill:
            if let target = action.target {
    ...
    private struct BattleActionSection: View {
        let item: ActionItem
        ...
        var body: some View {
            Section {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("\(item.actorName) (\(item.actorHP)/\(item.actorMaxHP))")
                            .font(.caption)
                            .foregroundStyle(.secondary)
